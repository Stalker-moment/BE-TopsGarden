...
import sendOutput from "../functions/sendOutput.js";
import encryptData from "../helper/encyptJson.js";

async function handleDataOutputSocket(ws, req) {
  ...
  const sendUpdatedData = async () => {
    const newData = await sendOutput();

    if (JSON.stringify(newData) !== data) {
      data = JSON.stringify(newData);
      //console.log("Data updated:", newData[0].states);
...
export default handleDataOutputSocket;